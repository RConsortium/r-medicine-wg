...
fewer choices for an online conferencing platform.  With help from the
...
that the user interface, consisting of a single-track video feed and sidebar participant chat worked well. One downside of Crowdcast is that it did not support screen readers or closed captioning.
...
* Although Crowdcast allows for smooth transitioning between speakers, time to transition between presenters needs to be built into the schedule, since people run over, technical issues emerge, and bio-breaks are important.  It is also wise to build in some flex-time in case there are technical difficulties.     

* During the presentations it was challenging to let speakers know when they were running out of time. All moderators downloaded a doorbell app to their phones that they played at pre-specified times, however that was not always sufficient. For 2021 we are planning to require that lightning talks be pre-recorded. 
